CREATE OR REPLACE FORCE VIEW app_auth_grids_v AS
WITH x AS (
    SELECT /*+ MATERIALIZE */
...
i AS (
    SELECT /*+ MATERIALIZE */
        ...
        --
    FROM t
    CROSS JOIN TABLE(APEX_STRING.SPLIT(app.get_user_views_text(t.source_object), CHR(10))) p
...
SELECT
    ...
    CASE
        WHEN i.page_items > 0 AND i.missing_items IS NOT NULL
            THEN app.get_icon_warning('Items ' || i.missing_items || ' missing on IG for proper refresh')
        WHEN i.page_items > 0
            THEN app.get_icon_check(i.page_items || ' page items used in view')
    ...
    --
    --
    -- count columns, PKs, R/O, ...? virtual cols
    --
    d.count_da
FROM t
/*
